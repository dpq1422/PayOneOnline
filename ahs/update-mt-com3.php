...
$min_record=0;
$min_query="SELECT min(mmt_id) mid FROM $bankapi_parent_txn.txn_mt where mmt_status in(1,3) and source=3 and type=1 order by mmt_id desc limit 0,1";
...
$qry_mt_com="SELECT * FROM $bankapi_parent_txn.txn_mt where source=3 and type=1 and mmt_status='2' and mmt_id>'$min_record' order by mmt_id desc;";
...
mysql_query("delete FROM $bankapi_child_txn.com_generated where source=3 and type=1 and order_id not in(SELECT distinct(order_id) FROM $bankapi_child_txn.com_paid_child where source=3 and type=1)");
?>