...
$min_record=0;
$min_query="SELECT min(mmt_id) mid FROM $bankapi_parent_txn.txn_mt where mmt_status in(1,3) and source=1 and type=2 order by mmt_id desc limit 0,1";
...
$qry_av_com="SELECT * FROM $bankapi_parent_txn.txn_mt where source=1 and type=2 and mmt_status='2' and mmt_id>100000 order by mmt_id desc;";
...
while($rs_av_com=mysql_fetch_array($res_av_com))
{
	...
	if($order_paid==0 || $mmt_paid==0)
	{
		...
		$client_qry="SELECT * FROM $bankapi_parent_base.charges_out_service WHERE mt_source_id='$source' and client_id='$clientid' and service_id='$service' and operator_id='$operator';";
