...
$min_record=0;
$min_query="SELECT * FROM $bankapi_parent_txn.txn_rc where mrc_status in(1,3) and source=4 and type in(3,4,5) order by mrc_id desc limit 0,1";
...
$qry_mt_com="SELECT * FROM $bankapi_parent_txn.txn_rc where source=4 and type in(3,4,5) and mrc_status='2' and mrc_id>10001800 order by mrc_id;";
...
while($rs_mt_com=mysql_fetch_array($res_mt_com))
{
	...
	$operator=show_operator_ids($operator,$source,$service);
	...
	if($order_paid==0 || $mmt_paid==0)
	{
		...
		$client_qry="SELECT * FROM $bankapi_parent_base.charges_out_service WHERE mt_source_id='$source' and service_id='$service' and operator_id='$operator' and client_id='$clientid';";// and mt_source_id='$source' //
