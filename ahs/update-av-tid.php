...
mysql_query("update $bankapi_parent_txn.txn_mt set mmt_status=2 WHERE source=1 and TYPE =2 and mmt_id>76000 and tid>600000000 AND mmt_status NOT IN ( 2, 5 );");
mysql_query("update $table.txn_mt_child set order_status=2 WHERE source=1 and TYPE =2 and order_id>76000 and tid>600000000 AND order_status NOT IN ( 2, 5 );");

mysql_query("update $bankapi_parent_txn.txn_mt set mmt_status=2 WHERE source =1 AND TYPE =2 AND mmt_id>76000 AND response_message='Success!  Account details found..';");
mysql_query("update $table.txn_mt_child set order_status=2 WHERE source =1 AND TYPE =2 AND order_id>76000 AND response_message='Success!  Account details found..';");

mysql_query("update $bankapi_parent_txn.txn_mt set mmt_status=-1 WHERE ( response LIKE '%cURL Error%' OR response LIKE '%Connection%' OR response LIKE '%invalid%' ) AND mmt_status =2 AND TYPE =1 AND mmt_id>76000;");

$qry_av_tid="SELECT * FROM $bankapi_parent_txn.txn_mt where source=1 and type=2 and response is not null and response!='' and (tid='0' or tid ='' or tid is null) and mmt_id>75000 order by mmt_id desc;";
