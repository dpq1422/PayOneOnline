...
function show_loginsliders_count($cond)
{
	...
	$query="select * from $bankapi_parent_base.parent_branding_login $cond order by slider_status desc, slider_id ";
...
function show_loginsliders_data($cond, $start_from=0, $num_rec_per_page=0)
{
	...
	$query="select * from $bankapi_parent_base.parent_branding_login $cond order by slider_status desc, slider_id LIMIT $start_from, $num_rec_per_page";
	$result=mysql_query($query);
	return $result;
}
?>