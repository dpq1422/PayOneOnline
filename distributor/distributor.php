											...
											<table>
												...
												<tr>
													<td align="left">Designation / Hierarchy <b style='color:red'>*</b><br>
														<select name='HierarchyName' required>
															<option value=''>Select Designation / Hierarchy</option>
															<?php 
															$query="SELECT hierarchy_id,hierarchy_name FROM child_hierarchy where hierarchy_id not in(0,1,11) and hierarchy_id>$user_type and status=1 order by hierarchy_id";
															$result=mysql_query($query);
															$num_rows = mysql_num_rows($result);
															if($num_rows>0)
															{
