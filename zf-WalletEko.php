...
function show_realtime_count($cond)
{
	...
	$query="select * from $bankapi_parent_wallet.rt_eko $cond ";
...
function show_realtime_data($cond, $start_from=0, $num_rec_per_page=0)
{
	...
	$query="select * from $bankapi_parent_wallet.rt_eko $cond order by wallet_id desc ";
	else
	$query="select * from $bankapi_parent_wallet.rt_eko $cond order by wallet_id desc LIMIT $start_from, $num_rec_per_page";
	$result=mysql_query($query);
	return $result;
}
?>