...
function show_retailers_data($cond, $start_from=0, $num_rec_per_page=0)
{
	...
	$query="select * from $bankapi_child_base.child_user $cond and user_type=12 order by user_id ";
	else
	$query="select * from $bankapi_child_base.child_user $cond and user_type=12 order by user_id  LIMIT $start_from, $num_rec_per_page";
	$result=mysql_query($query);
	return $result;
}
