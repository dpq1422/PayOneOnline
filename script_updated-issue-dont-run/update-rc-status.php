...
$qry="update $bankapi_parent_txn.txn_rc set mrc_status='4' where response like '%Operator temporarily unavailable.%' and mrc_status in (1,3)";
mysql_query($qry);
$qry="update $bankapi_parent_txn.txn_rc set mrc_status='4' where response like '%Order number do not exist%' and mrc_status in (1,3)";
mysql_query($qry);

$min_record=100001150;
$min_query="SELECT * FROM $bankapi_parent_txn.txn_rc where mrc_status in(1,3) and source=2 and type in(3,4) order by mrc_id limit 0,1";
...
$query="SELECT * FROM $bankapi_parent_txn.txn_rc where mrc_status in(1,3) and source=2 and type in(3,4) and (result is null or result='0' or result='') and mrc_id>$min_record order by mrc_id asc";
...
$query="SELECT * FROM $bankapi_parent_txn.txn_rc where mrc_status in(1,3) and source=2 and type in(3,4) and (result is not null and result!='0' and result!='') and mrc_status>$min_record order by mrc_id asc";
$result=mysql_query($query);
$i=0;
while($rs = mysql_fetch_array($result))
{	
