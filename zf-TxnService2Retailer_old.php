...
function show_txn_count($cond)
{
	...
	$query="select * from $bankapi_child_txn.txn_rc $cond ";
...
function show_txn_data($cond, $start_from=0, $num_rec_per_page=0)
{
	...
	$query="select * from $bankapi_child_txn.txn_rc $cond order by rc_id desc ";
	else
	$query="select * from $bankapi_child_txn.txn_rc $cond order by rc_id desc LIMIT $start_from, $num_rec_per_page";
	$result=mysql_query($query);
	return $result;
}
?>