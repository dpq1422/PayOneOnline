...
function abcd(row_no)
{
	...
	var req_user=$("#req_user"+row_no).val();
	var status=$("#status"+row_no).val();
	var remarks=$("#remarks"+row_no).val();
	var userid=$("#userid"+row_no).val();
	var username=$("#username"+row_no).val();
	//alert(status);
	$.ajax({
		type: "POST",
		url: "AjaxTicketReceivedUpdateServlet",
		data: {'req_user': req_user , 'req': req , 'status': status , 'remarks': remarks , 'userid': userid , 'username': username },
		dataType: "json",
            ...
            <div class="w3-row-padding w3-margin-top">
            	<div class="w3-col m12 wow bounceIn">
                	<div class="table-box wh w3-left">
						...
						?>
                    	<div class="box-head">
                        	<h3>LIST OF TICKETS (RECEIVED) <span class="w3-right w3-blue w3-center badges"><?php echo $total_records;?></span></h3>
                        </div>
                        	...
                        	<ul>
								...
								while($user_row=mysql_fetch_array($user_result))
								{
									...
									if($ticket_status==1)
									{
										...
										$st_link="<input type='hidden' id='req$i' value='".$user_row['ticket_id']."' /><input type='hidden' id='req_user$i' value='".$user_row['user_id']."' /><input type='hidden' id='userid$i' value='$logged_user_id' /><input type='hidden' id='username$i' value='$logged_user_name' />
										...
										<table class="wh">
											<tr>
												<td width="35%"valign="top">
													<p><strong>CLIENT REMARKS:-</strong> <?php echo $user_row['user_remarks'];?></p>
													<p><strong>CLIENT TIME:-</strong> <?php echo $user_row['user_updates'];?></p>
												</td>
												<td width="35%" valign="top">
