							...
							<div class="col-md-12">
								<div class="panel panel-primary">
									<div class="panel-heading bgheadcolor">
										Transactions (Real Time Admin Wallet)
									</div>
								<form method="post">
									<?php											
									$s1="";
									...
									<div class="panel-body panel-primary text-left">
										...
										&nbsp;&nbsp;&nbsp;<input type='number' size="30" value="<?php echo $s2;?>" placeholder="Order ID" id='b' name='b' onkeyup='clearother(2)' />
										...
										<table class="table clsgrid rounded_corners" cellspacing="0" rules="all" border="1" id="ctl00_ContentPlaceHolder1_tagcost" style="width:100%;border-collapse:collapse;font-family:Calibri;font-weight:500;">
											<tbody>
												<tr class="gridheader" align="center" style="background-color:#009DE2;">
													<th>S.No.</th>
													<th>Wallet ID</th>
													<th>Order ID<br>Group ID<br>Txn ID</th>
												...
												<?php
												$query="SELECT * FROM child_wallet_realtime where client_order_id!=0 $cond and transaction_description not like 'RC order %' order by wallet_id desc limit 0,100";
												...
												if($num_rows>0)
												{
													...
													while($rs = mysql_fetch_array($result))
													{
														...
														if($ttype==1)
														$ttype="<b style='color:green;'>Wallet Received</b>";
														if($ttype==2)
														$ttype="Order Generated";
														if($ttype==3)
														$ttype="<b style='color:#cc5801;'>Failed Order Refunded</b>";
														...
														$st="";
														$refno="";
														$eid=$rs['client_order_id'];
														
														$qry1a="select * from main_transaction_mt where eko_transaction_id='$eid'";
														$res1a=mysql_query($qry1a);
														while($rs1a=mysql_fetch_array($res1a))
														{
															$tp=$rs1a['channel_desc'];
															$rcvid=$rs1a['receiver_id'];
															$grp=$rs1a['group_id'];
															$tid=$rs1a['tid'];
															$st=$rs1a['eko_transaction_status'];
															$refno=$rs1a['bank_ref_no'];
														}
														...
														//$ttype=$ttype."<br>".$tp."<br>".$bnk;
														//$tdesc=$rs['transaction_description']."<br>".$st;
												?>
												<tr style="background-color:White;">
													<td><?php echo $i;?></td>
													<td><?php echo $rs['wallet_id'];?></td>
													<td><?php echo $rs['client_order_id']."<br>(".$grp.")<br>".$tid;?></td>
													<td><?php echo $uuiidd;?><br><br><?php echo show_my_uname($uuiidd);?></td>
