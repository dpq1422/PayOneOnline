							...
							<div class="col-md-12">
								<div class="panel panel-primary">
									...
									<div class="panel-body panel-primary text-left">
										...
										<input type='button' value='Download Excel' onclick="window.location.href='wallet-request-received.php<?php echo $dts;?>';" style='float:right;' />
										</form>
									</div>
									<div class="panel-body panel-primary text-center">
										<table class="table clsgrid rounded_corners" cellspacing="0" rules="all" border="1" id="ctl00_ContentPlaceHolder1_tagcost" style="width:100%;border-collapse:collapse;font-family:Calibri;font-weight:500;">
											<tbody>
												<tr class="gridheader" align="center" style="background-color:#009DE2;">
													<th>Sr.No.</th>
													<th>Req ID</th>
													<th>Requset Date/Time<br>Deposit Date</th>
													<th>User Name (ID)</th>
												...
												$query="SELECT * FROM child_wallet_requests where bank_id<100000 $cond ";
												$result=mysql_query($query);
												$num_rows = mysql_num_rows($result);	
												if($num_rows>0)
												{				
											...
											<tr <?php echo $style;?>>
												...
												<td>R: <?php echo $rs['request_date']."<br>".$rs['request_time'];?><br><br>D: <?php echo $rs['deposite_date'];?></td>
												<td><?php echo show_my_uname($rs['user_id'])." (".$rs['user_id'].")<br>".show_my_umobile($rs['user_id']);?></td>
												<td><?php echo show_admin_bank_client(1001,$rs['bank_id']);?></td>
												<td><?php echo $pm;?></td>
