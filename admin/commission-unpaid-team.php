					...
					?>
									...
									<div class="panel-body panel-primary text-center">
										<table class="table clsgrid rounded_corners" cellspacing="0" rules="all" border="1" id="ctl00_ContentPlaceHolder1_tagcost" style="width:100%;border-collapse:collapse;font-family:Calibri;font-weight:500;">
											...
											<?php
											$query="SELECT user_id id,sum(cr) cr,sum(dr) dr, (sum(cr)-sum(dr)) bal FROM main_commission_paid where user_id not in (1,100000,100001) $cond group by user_id order by bal desc limit 0,100";
											...
											if($num_rows>0)
											{
												...
												while($rs = mysql_fetch_assoc($result))
												{
													...
													$status="<a href='commission-unpaid-teams.php?uid=$uid'>Show Details</a><br><br><a href='commission-unpaid-teams-dr.php?uid=$uid'>Paid Only</a>";
													if($bal>100)
													{
														$status=$status."<br><br>";
